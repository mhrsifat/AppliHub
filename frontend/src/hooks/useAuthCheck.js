// filepath: src/hooks/useAuthCheck.js
import { useEffect, useState, useRef } from "react";
import { useDispatch, useSelector } from "react-redux";
import api, { setAccessToken, logout } from "../services/api";
import { setUser, setEmployee, setAdmin } from "../features/auth/slices/authSlice";
    ...
    const init = async () => {
      try {
        ...
        if (!user && !employee && !admin) {
          ...
          if (newToken) {
  await Promise.resolve(setAccessToken(newToken));
}
        ...
        await logout(false); // soft logout — no redirect
      } finally {
        setLoading(false);
      }
    };
