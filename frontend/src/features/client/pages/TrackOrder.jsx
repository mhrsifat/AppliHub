...
export default function TrackOrder() {
  ...
  const search = async (e) => {
    ...
    try {
      const res = await axios.get("/api/public/track-order", { params: { q: query.trim() } });
      setData(res.data);
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      {!data && !loading && (
        <div className="text-sm text-gray-600">Enter your order number or invoice number to see status and payment options.</div>
      )}
            ...
            <div className="mb-6 border rounded p-4 bg-white shadow-sm">
              ...
              <div className="grid grid-cols-3 gap-4 mt-3 text-sm">
                ...
                <div>
                  ...
                  <div>{data.order.customer_email} {data.order.customer_phone ? ` / ${data.order.customer_phone}` : ''}</div>
                </div>
                <div>
          ...
          {data.summary && (
            <div className="mb-6 grid grid-cols-3 gap-4">
              <div className="p-4 bg-white rounded shadow-sm text-center">
                ...
                <div className="text-lg font-semibold">{data.summary.total_payable.toFixed ? data.summary.total_payable.toFixed(2) : data.summary.total_payable} BDT</div>
              </div>
              <div className="p-4 bg-white rounded shadow-sm text-center">
                <div className="text-sm text-gray-500">Total Paid</div>
                <div className="text-lg font-semibold">{data.summary.total_paid.toFixed ? data.summary.total_paid.toFixed(2) : data.summary.total_paid} BDT</div>
              </div>
              <div className="p-4 bg-white rounded shadow-sm text-center">
                <div className="text-sm text-gray-500">Total Due</div>
                <div className="text-lg font-semibold">{data.summary.total_due.toFixed ? data.summary.total_due.toFixed(2) : data.summary.total_due} BDT</div>
              </div>
            </div>
          )}
              ...
              data.invoices.map((inv) => (
                <div key={inv.id} className="p-4 bg-white border rounded shadow-sm">
                  ...
                  <div className="mt-4 flex items-center gap-2">
                    <button onClick={() => downloadPdf(inv)} className="px-3 py-2 bg-gray-800 text-white rounded text-sm">Download PDF</button>

                    {inv.status !== 'paid' && (data.pay_options?.sslcommerz?.enabled) ? (
                      <button onClick={() => openPayModal(inv)} className="px-3 py-2 bg-blue-600 text-white rounded text-sm">Pay Now</button>
                    ) : null}

                    {/* show quick breakdown */}
                    <div className="ml-auto text-sm text-gray-600">
                      Paid: <strong>{Number(inv.paid_amount ?? (inv.payments ? inv.payments.reduce((s,p)=> s + (p.status === 'completed' ? Number(p.amount) : 0),0) : 0)).toFixed(2)} BDT</strong>
                      &nbsp;|&nbsp;
                      Due: <strong>{(Number(inv.grand_total) - Number(inv.paid_amount ?? (inv.payments ? inv.payments.reduce((s,p)=> s + (p.status === 'completed' ? Number(p.amount) : 0),0) : 0))).toFixed(2)} BDT</strong>
                    </div>
                  </div>
                </div>
              ))
      ...
      {payInvoice && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded p-6 w-full max-w-lg">
            ...
            <p className="text-sm text-gray-600 mb-3">Amount: <strong>{Number(payInvoice.grand_total).toFixed(2)} BDT</strong></p>
